...
class NumberPickerView: ConstraintLayout {
    ...
    init {
        inflate(context, R.layout.widget_number_picker, this)
    }
    ...
    private fun init(attrs: AttributeSet?){
        attrs?.let { attr ->
            val attributes = context.obtainStyledAttributes(attr, R.styleable.NumberPickerView)
            try{
                defaultText = attributes.getString(R.styleable.NumberPickerView_label) ?: defaultText
                remoteKey = attributes.getString(R.styleable.NumberPickerView_remoteLabel) ?: remoteKey
                limit = attributes.getInt(R.styleable.NumberPickerView_maxNumber, limit)
                defaultNumber = attributes.getInt(R.styleable.NumberPickerView_defaultNumber, 0)
                inferiorLimit = attributes.getInt(R.styleable.NumberPickerView_inferiorLimit, inferiorLimit)
    ...
    fun setDefaultNumber(default: Int, notifyListener: Boolean = true){
        if(notifyListener) numberPickerListener?.onChangeNumber(default)
