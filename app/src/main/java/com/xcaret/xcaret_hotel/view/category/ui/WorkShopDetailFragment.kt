...
class WorkShopDetailFragment: BaseFragmentDataBinding<WorkShopDetailViewModel, WorkshopDetailFragmentBinding>() {
    override fun getLayout(): Int = R.layout.workshop_detail_fragment
    ...
    private fun setUpDays(){
        try {
            ...
            days.forEachIndexed { index, dayOfWeek ->
                val day = DayBooking()
                day.dayName =
                    (days[index].getDisplayName(TextStyle.SHORT, Locale.getDefault())).lowercase()
    ...
    private fun setObservers(){
        ...
        viewModel.activityWithSchedules().observe(this, Observer { schedules ->
            ...
            removeDays()
            /*doAsync {
                val listGroupSchedule = mutableListOf<String>()
                schedules.forEach { schedule ->
                    val newFormatedDateStart = DateUtil.checkDeviceTimeFormat(HotelXcaretApp.mContext,schedule.hourStart)
                    val newFormatedDateEnd = DateUtil.checkDeviceTimeFormat(HotelXcaretApp.mContext,schedule.hourEnd)

                    //val s = "${schedule.hourStart} - ${schedule.hourEnd}"
                    val s ="$newFormatedDateStart - $newFormatedDateEnd"
                    if(s.isNotEmpty() && !listGroupSchedule.contains(s))
                        listGroupSchedule.add(s)
                }
                val formatSchedule = listGroupSchedule.joinToString(separator = " - ")
                uiThread {
                    binding.txtSchedule.text = formatSchedule
                }
            }*/
        })
    }
