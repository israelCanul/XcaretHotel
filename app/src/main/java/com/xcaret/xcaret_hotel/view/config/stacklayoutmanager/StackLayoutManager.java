...
import static com.xcaret.xcaret_hotel.view.config.stacklayoutmanager.Align.BOTTOM;
import static com.xcaret.xcaret_hotel.view.config.stacklayoutmanager.Align.LEFT;
import static com.xcaret.xcaret_hotel.view.config.stacklayoutmanager.Align.RIGHT;
import static com.xcaret.xcaret_hotel.view.config.stacklayoutmanager.Align.TOP;
...
public class StackLayoutManager extends RecyclerView.LayoutManager {
    ...
    private void brewAndStartAnimator(int dur, int finalXorY) {
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                final int currentPos = mTotalOffset / mUnit;
                if(itemChangeListener != null) itemChangeListener.onItemChange(recycler.getViewForPosition(currentPos), currentPos);
                lastAnimateValue = 0;
            }
    ...
    private void brewAndStartAnimator(int dur, int finalXorY, boolean trigger) {
        animator = ObjectAnimator.ofInt(StackLayoutManager.this, "animateValue", 0, finalXorY);
        animator.setDuration(dur);
        animator.start();
        animator.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                final int currentPos = mTotalOffset / mUnit;
                if(itemChangeListener != null && trigger) itemChangeListener.onItemChange(recycler.getViewForPosition(currentPos), currentPos);
                lastAnimateValue = 0;
            }
...
//                        -(position - currPos)*(mItemWidth) * (1 - closestBaseItemScale));
    ...
    public void scrollToPosition(int position, boolean trigger) {
        if (position > getItemCount() - 1) {
            Log.i(TAG, "position is " + position + " but itemCount is " + getItemCount());
            return;
        }
